...
- **主助手**：入口，自带 `search_flights`（航班搜索）和 `lookup_policy`（FAQ 政策 RAG 检索），并根据用户意图路由到对应子助手
- **子助手**：各自负责对应领域的读写操作，每个子助手内部也分 safe tools（搜索，直接执行）和 sensitive tools（预订/取消/修改，需人工审批）
- **安全机制**：所有写操作在 LLM 生成工具调用后挂起中断，用户输入 `y` 确认后才真正执行
